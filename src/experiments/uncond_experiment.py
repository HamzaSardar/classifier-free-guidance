...
from src.cfg_diffusion.uncond_diffusion import ContinousDiffusion
    ...
    for epoch in range(n_epochs):
        for idx, zipped_data in enumerate(zip(train_loader, itertools.cycle(val_loader))):
            ...
            with accelerator.accumulate(diffusion):
                ...
                y = data[0]
                loss = diffusion(standardise_fn(y))
            ...
            with torch.no_grad():
                y = val_data[0]
                val_loss = diffusion(standardise_fn(y))
