...
class ContinuousForwardDiffusion(nn.Module):
    ...
    def forward(self, y0: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        y_noisy, eps_hat, eps: tuple[Tensor, Tensor, Tensor]
            Noised image, predicted noise, and actual noise.
        """
        y_noisy, log_snr_sample, eps = self.noising(y0)

        eps_hat = self.denoise_model(y_noisy, log_snr_sample.view(-1, 1, 1, 1))
...
class ContinuousReverseDiffusion(nn.Module):
    ...
    def reverse_mean_variance(self, t: int, y_lambda: Tensor) -> tuple[Tensor, Tensor]:

        eps_hat = self.denoise_model(y_lambda, self.lambdas[t].view(-1, 1, 1, 1).to(y_lambda.device))
    ...
    def reverse_sample(self, t: int, y_lambda: Tensor) -> Tensor:
        mean, variance = self.reverse_mean_variance(t, y_lambda)
...
class ContinousDiffusion(nn.Module):
    ...
    def forward(self, y: Tensor) -> Tensor:
        ...
        _, eps_hat, eps = self.forward_process(y)
